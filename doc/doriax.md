...
**Format de représentation.** _DORIAX n'impose aucun format de représentation — du JSON ordinaire convient. Mais DORIAX et **HDL** (Hypermedia Domain Language, spécifié séparément, voir [HDL](README.md)) forment le couple idéal, et c'est ce couple que cette section illustre. HDL est un profil hypermédia bâti sur HAL : il en reprend la structure (`_links`, `self`, `href`) et l'enrichit (méthode HTTP portée par le lien, titres, dépréciations, templates d'URI). Les exemples de payload de cette section n'en utilisent qu'un sous-ensemble — les `_links` de base — et restent donc du HDL valide. Dans ce cadre, la représentation des erreurs suit également HDL (voir [Erreurs](doc/errors.md))._
...
**Note — format d'erreur.** _Côté erreurs, DORIAX recommande la **RFC 9457** (« Problem Details for HTTP APIs », qui a remplacé la RFC 7807 en juillet 2023), servie en `application/problem+json` : champs `type`, `title`, `status`, `detail`, `instance`. Dans le couple avec HDL, ce socle est étendu par le **format d'erreur HDL**, un sur-ensemble de la RFC 9457 qui ajoute un code applicatif stable (`code`), des erreurs multiples (`errors[]`), des liens de reprise (`_links`) et des métadonnées d'échange (`_metadata`). L'exemple ci-dessus n'en montre qu'une forme simple ; voir [Erreurs](doc/errors.md) pour la spécification complète._
...
**Au niveau HTTP, `Prefer` n'est qu'une préférence** : la RFC 7240 autorise le serveur à l'ignorer. **DORIAX durcit ce point au niveau du profil** — un serveur DORIAX conforme **honore** `return=representation` et renvoie alors la représentation complète et à jour de la ressource ; le client peut donc s'y fier sans avoir à coder le cas contraire. L'exigence ne vaut **que** pour une opération qui s'achève de façon **synchrone** sur une cible **représentable** : pour un traitement asynchrone (`202 Accepted`, voir §3.3) ou une écriture sans résultat re-lisible (`204 No Content`, ou la classe « le corps est le résultat » décrite plus bas), la préférence est **sans objet** et le serveur répond comme à l'ordinaire. Lorsqu'il l'honore, le serveur le confirme par `Preference-Applied: return=representation`. `return=minimal` demeure le **défaut** — inutile à envoyer, sauf pour l'expliciter —, et c'est le défaut que DORIAX documente ici.
...
C'est ici aussi que [HDL](../README.md) referme le sujet proprement. Plutôt que de laisser le client *deviner* quelle URL relire après une écriture, la réponse lui fournit les **liens** vers ce qu'il peut consulter ou faire ensuite : la relecture devient un lien donné par le serveur, pas une URL reconstruite côté client. La séparation commande/requête et HDL répondent ainsi au même manque. Surtout, ces liens font plus que router : le serveur n'expose le lien d'une action que si elle est, pour cet acteur et dans l'état courant, permise et valide — les affordances annoncées portent donc les permissions et les règles métier à l'instant T, que le client n'a plus à redupliquer.
